      ...
      <div className="bg-surface-container backdrop-blur-md border border-surface-variant p-5 lg:p-6 rounded-3xl space-y-4 transition-all duration-300 shadow-sm dark:shadow-none">
        <div className="flex flex-col sm:flex-row gap-3">
          <input
            ...
            className="flex-1 bg-surface-container border border-outline-variant focus:border-zinc-500 dark:focus:border-zinc-600 rounded-2xl px-5 py-4 text-xs font-sans placeholder-zinc-400 dark:placeholder-zinc-500 text-on-surface outline-none focus:ring-1 focus:ring-zinc-300 dark:focus:ring-zinc-700/50 transition-all shadow-sm dark:shadow-none w-full"
            disabled={loading}
          />
          <button
            onClick={() => handleOnSubmit("")}
            disabled={loading || !userInput.trim()}
            className="px-6 py-4 rounded-2xl bg-zinc-900 hover:bg-zinc-800 dark:bg-zinc-100 dark:hover:bg-white text-white dark:text-zinc-900 font-semibold text-xs flex items-center justify-center gap-2 transition-all disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer shadow-md dark:shadow-none w-full sm:w-auto shrink-0"
          >
        ...
        <div className="space-y-1.5">
          <span className="text-[10px] font-mono text-on-surface-variant dark:text-zinc-400 uppercase tracking-widest block font-bold">
          ...
          <div className="flex flex-wrap gap-2 w-full">
            {PRESET_EXCUSAS.map((preset, i) => (
              <button
                ...
                disabled={loading}
                className="text-xs px-3.5 py-2.5 rounded-xl bg-surface-container hover:bg-surface hover:text-on-surface dark:hover:bg-zinc-800/80 dark:hover:text-zinc-200 text-on-surface-variant border border-surface-variant transition-all flex items-start gap-2 hover:border-zinc-500 dark:hover:border-zinc-600 text-left cursor-pointer shadow-sm dark:shadow-none w-full sm:w-auto whitespace-normal break-words"
              >
                <PlusCircle className="w-3.5 h-3.5 text-on-surface-variant dark:text-zinc-400 shrink-0 mt-0.5" />
      ...
      <AnimatePresence>
        {error && (
          <motion.div
            ...
            className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900/50 rounded-2xl p-5 flex gap-4 text-xs shadow-sm dark:shadow-none"
          >
            <div className="bg-red-100 dark:bg-red-900/50 p-2 rounded-full h-fit">
              <AlertCircle className="w-5 h-5 text-red-600 dark:text-red-400 shrink-0" />
            </div>
            <div className="space-y-1.5 flex-1 pt-0.5">
              <span className="font-bold text-red-800 dark:text-red-300 block text-[13px]">Sistema IA no disponible temporalmente</span>
              <p className="text-red-600/90 dark:text-red-400/90 leading-relaxed">
                {error.includes("API key")
                  ? "Para utilizar el descompresor socrático localmente, necesitas configurar tu propia GEMINI_API_KEY en el archivo .env. Puedes obtener una gratuita en Google AI Studio."
                  : "El motor de procesamiento cognitivo de Sintiens no pudo analizar tu premisa en este momento. " + error}
              </p>
            </div>
          </motion.div>
        )}
            ...
            <div className="border border-surface-variant rounded-3xl overflow-hidden bg-surface-container shadow-sm dark:shadow-none transition-all duration-300">
              ...
              <div className="bg-surface border-b border-surface-variant p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 transition-colors">
                <div className="space-y-1.5">
                  <span className="text-[10px] font-mono tracking-widest text-on-surface-variant uppercase flex items-center gap-1 font-bold">
                    <Workflow className="w-3.5 h-3.5 text-on-surface-variant" /> DIAGNÓSTICO DIALÉCTICO SINTIENS
                  </span>
                  <h3 className="text-lg font-bold text-on-surface tracking-tight leading-snug font-serif">
                    &ldquo;{analysis.argumentSummary}&rdquo;
                  </h3>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-mono px-2.5 py-1 rounded bg-purple-50 dark:bg-purple-500/20 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-500/30 font-semibold shadow-sm">
              ...
              <div className="p-6 grid grid-cols-1 lg:grid-cols-12 gap-8 text-xs font-light">
                ...
                <div className="lg:col-span-5 space-y-4">
                  ...
                  {/* Axiomas list */}
                  <div className="space-y-2 bg-surface p-5 rounded-2xl border border-surface-variant transition-all shadow-sm dark:shadow-none">
                    <h4 className="text-xs font-semibold tracking-wider font-mono text-on-surface uppercase flex items-center gap-1.5 border-b border-surface-variant pb-2 mb-3 transition-colors">
                      <ShieldAlert className="w-4 h-4 text-orange-600 dark:text-orange-400" />
                      Axiomas Implícitos No Examinados
                    </h4>
                    <p className="text-[11px] text-on-surface-variant leading-relaxed mb-4 transition-colors">
                    ...
                    <ul className="space-y-2">
                      {analysis.axioms.map((ax, i) => (
                        <li key={i} className="flex items-start gap-2.5 text-on-surface-variant bg-surface-container px-4 py-3 rounded-xl border border-surface-variant transition-colors shadow-sm dark:shadow-none">
                          <CheckCircle2 className="w-4 h-4 text-zinc-400 dark:text-zinc-500 shrink-0 mt-0.5" />
                          <span className="leading-relaxed text-[11.5px]">{ax}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div className="space-y-2 bg-surface p-5 rounded-2xl border border-surface-variant transition-all shadow-sm dark:shadow-none">
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      <span className="text-[10px] font-mono px-3 py-1 rounded-md uppercase font-bold bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 transition-colors">
                        Calificación: {analysis.scientificAccuracy.rating}
                      </span>
                    </div>
                    <p className="text-[11.5px] text-on-surface-variant leading-relaxed font-light transition-colors">
                ...
                <div className="lg:col-span-7 space-y-6 flex flex-col justify-between">
                  ...
                  <div className="space-y-2 bg-surface p-5 rounded-2xl border border-surface-variant transition-all shadow-sm dark:shadow-none">
                    ...
                    <div className="space-y-3.5 font-light text-on-surface-variant transition-colors">
                      {analysis.logicalFailures.map((fail, i) => (
                        <p key={i} className="leading-relaxed text-[11.5px] bg-zinc-50/50 dark:bg-zinc-900/30 p-3 rounded-xl border border-zinc-100 dark:border-zinc-800/50">
                          {fail}
                        </p>
                      ))}
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    {/* Animal suffering */}
                    <div className="bg-surface p-5 rounded-2xl border border-surface-variant space-y-3 transition-colors shadow-sm dark:shadow-none">
                      <div className="flex items-center gap-1.5 text-red-600 dark:text-red-400 font-semibold font-mono text-[10px] tracking-wider uppercase transition-colors border-b border-surface-variant pb-2">
                        <Activity className="w-4 h-4 text-red-600 dark:text-red-500 animate-pulse" />
                    ...
                    {/* Green impact */}
                    <div className="bg-surface p-5 rounded-2xl border border-surface-variant space-y-3 transition-colors shadow-sm dark:shadow-none">
                      <div className="flex items-center gap-1.5 text-emerald-600 dark:text-emerald-400 font-semibold font-mono text-[10px] tracking-wider uppercase transition-colors border-b border-surface-variant pb-2">
              ...
              <div className="bg-surface p-8 border-t border-surface-variant text-center space-y-4 relative overflow-hidden transition-colors">
                <div className="absolute left-6 top-1/2 -translate-y-1/2 opacity-[0.03] dark:opacity-[0.02] text-on-surface pointer-events-none">
                  <Brain className="w-32 h-32" />
                </div>
                <div className="flex items-center justify-center gap-2 text-on-surface-variant text-[10px] font-mono tracking-widest uppercase font-bold">
                  <Sparkles className="w-4 h-4 text-amber-600 dark:text-amber-500" />
                  PREGUNTA SOCRÁTICA FINAL SINTIENS
                </div>
                <p className="text-base md:text-lg font-serif italic text-on-surface dark:text-zinc-100 max-w-3xl mx-auto leading-relaxed transition-colors z-10 relative">
